...
exports.getAll = function(offset, limit, callback){
	CustomerModel.find({__disabled : {$ne : true}, _id : {$ne : null}}).limit(limit).skip(offset).exec(callback);
};
...
exports.getHistory = function(id, callback){
	CustomerModel.find({_id : id}, callback);
};

exports.rollback = function(id, versionNumber, callback){
	CustomerModel.findOne({_id : id, __v : versionNumber}, function(err, data){
		if(err || data == null){
			callback(err, data);
		}else{
